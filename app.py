...
<html><head>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Edit Product - Seller AI</title>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
<style>
*{{box-sizing:border-box;margin:0;padding:0;}}
body{{font-family:'Inter',sans-serif;background:#f0f2f8;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;}}
.card{{background:white;border-radius:24px;padding:40px;width:100%;max-width:560px;box-shadow:0 8px 32px rgba(0,0,0,0.1);}}
h1{{font-size:22px;font-weight:800;color:#0f172a;margin-bottom:24px;}}
label{{display:block;font-size:13px;font-weight:600;color:#374151;margin-bottom:6px;margin-top:16px;}}
input,textarea{{width:100%;padding:12px 16px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:14px;font-family:'Inter',sans-serif;color:#0f172a;outline:none;}}
input:focus,textarea:focus{{border-color:#7c3aed;box-shadow:0 0 0 3px rgba(124,58,237,0.1);}}
textarea{{height:120px;resize:vertical;}}
.btn-row{{display:flex;gap:12px;margin-top:24px;}}
.btn-save{{flex:1;padding:13px;background:linear-gradient(135deg,#1d4ed8,#7c3aed);color:white;border:none;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;}}
.btn-cancel{{padding:13px 20px;background:#f1f5f9;color:#64748b;border:none;border-radius:10px;font-size:15px;font-weight:600;cursor:pointer;}}
</style>
</head><body>
<div class="card">
...
def index():
        ...
        for i, (_, row) in enumerate(df.iterrows()):
            ...
            <div class="card {'duplicate-card' if is_duplicate else ''}" data-index="{i}" 
                ...
                <div class="card-body">
                    ...
                    <button onclick="window.location.href='/edit/{row['id']}'" class="blue" style="width:100%;padding:11px;border-radius:10px;font-size:13px;margin-top:5px;">✏️ Edit</button>
                    <button onclick="deleteProduct({row['id']})" class="orange" style="width:100%;padding:11px;border-radius:10px;font-size:13px;margin-top:5px;">🗑️ Delete</button>
                </div>
            </div>
            """
    ...
    <head>
        ...
        <style>
            ...
            .products {{ display: grid; grid-template-columns: repeat(auto-fill,minmax(320px,1fr)); gap: 24px; padding: 8px 32px 60px; justify-items: center; align-items: start; }}
            .card {{ background: white; border-radius: 22px; overflow: hidden; box-shadow: 0 8px 25px rgba(0,0,0,0.08); position: relative; transition: 0.3s ease; border: 1px solid #f0f0f0; width: 100%; max-width: 340px; display: flex; flex-direction: column; }}
            ...
            .product-image {{ width: 100%; height: 280px; object-fit: cover; background: #f7f7f7; }}
