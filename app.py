    ...
    if "drive.google.com" in url:
        try:
            ...
            if match:
                return f"https://lh3.googleusercontent.com/d/{match.group(1)}=s1200"
            if "id=" in url:
                return f"https://lh3.googleusercontent.com/d/{url.split('id=')[1].split('&')[0]}=s1200"
        except:
            return "https://placehold.co/600x400?text=Broken+Image"
    if "dropbox.com" in url:
        return url.replace("www.dropbox.com", "dl.dropboxusercontent.com").replace("?dl=0", "?raw=1")
    if any(x in url for x in ["1drv.ms", "onedrive.live.com", "daraz", "lazada", "alicdn",
                                "amazon.com", "amazonaws.com", "ssl-images-amazon", "shopify.com", "cdn.shopify"]):
        return url
...
def smart_detect_columns(df):
    title_keywords    = ['product title','product name','item name','name','title','product','item']
    desc_keywords     = ['description','details','product description','desc','about','specification','features']
    image_keywords    = ['image','photo','img','picture','image url','main image','image link','image1']
    price_keywords    = ['price','sale price','selling price','retail price','cost','amount','rate','mrp']
    category_keywords = ['category','type','product type','department','genre','class','group','collection']
    stock_keywords    = ['stock','quantity','qty','inventory','availability','available','units']
    ...
    if not title_col:
        for col in df.columns:
            ...
            if sample.str.len().mean() > 5:
                title_col = col
                break
        title_col = title_col or df.columns[0]

    if not price_col:
        for col in df.columns:
            try:
                numeric = pd.to_numeric(
                    df[col].astype(str).str.replace(',','').str.replace('rs','',case=False).str.strip(),
                    errors='coerce'
                )
...
def upload_to_cloudinary(image_url):
    ...
    try:
        result = cloudinary.uploader.upload(
            image_url, folder="seller_dashboard",
            resource_type="image", timeout=10
        )
...
def generate_sku(title):
    ...
    return f"{short}-{''.join(random.choices(string.digits, k=4))}"

def generate_tags(title, category):
    words = list(set(str(title).lower().split() + str(category).lower().split()))
    return ', '.join(words[:8])
...
def ai_optimize_product(title, description, category=""):
    ...
    bullet_points   = [
        "✔ High quality & durable build", "✔ Fast delivery all over Nepal",
        "✔ Trusted seller product",       "✔ Excellent value for money",
        "✔ Ideal for everyday usage"
    ]
    seo_tags = ", ".join([w.lower() for w in title.split() if len(w) > 3][:10])
    ...
    return {"optimized_title": optimized_title, "bullet_points": bullet_points,
            "optimized_description": optimized_description, "seo_tags": seo_tags}
...
"""
AUTH_HEAD = '<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet"><meta name="viewport" content="width=device-width, initial-scale=1.0">'
    ...
    if request.method == 'POST':
        ...
        if len(password) < 6:
            ...
            send_email(email, "Verify your Seller AI account",
                f"<div style='font-family:Arial;max-width:500px;margin:0 auto;padding:20px;'>"
                f"<h2 style='color:#1d4ed8;'>Welcome to Seller AI, {username}!</h2>"
                f"<p>Please verify your email to get started.</p>"
                ...
                f"<p style='color:#64748b;font-size:13px;'>If you did not register, ignore this email.</p></div>")
            return redirect('/login?msg=verify')
    error_html = f'<div class="error">&#x26A0; {error}</div>' if error else ''
    return f"""<!DOCTYPE html><html><head>{AUTH_HEAD}<title>Register - Seller AI</title>{AUTH_STYLE}</head><body>
<div class="card">
    <div class="logo"><div class="logo-icon">&#x1F6D2;</div><h1>Create Account</h1><p>Join Seller AI Dashboard</p></div>
    {error_html}
    <form method="POST">
        <label>Username</label><input name="username" placeholder="Your name" required>
...
def resend_verification():
    ...
    return f"""<!DOCTYPE html><html><head>{AUTH_HEAD}<title>Resend Verification - Seller AI</title>{AUTH_STYLE}</head><body>
<div class="card">
    <div class="logo"><div class="logo-icon">&#x2709;</div><h1>Resend Verification</h1><p>We'll send you a new link</p></div>
    {msg_html}
    <form method="POST">
        <label>Email</label><input name="email" type="email" placeholder="you@example.com" required>
...
def forgot_password():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            send_email(email, "Reset your Seller AI password",
                f"<div style='font-family:Arial;max-width:500px;margin:0 auto;padding:20px;'>"
                f"<h2 style='color:#1d4ed8;'>Reset Your Password</h2>"
    ...
    msg_html = f'<div class="info">&#x2709; {message}</div>' if message else ''
    return f"""<!DOCTYPE html><html><head>{AUTH_HEAD}<title>Forgot Password - Seller AI</title>{AUTH_STYLE}</head><body>
<div class="card">
    <div class="logo"><div class="logo-icon">&#x1F511;</div><h1>Forgot Password</h1><p>We'll send you a reset link</p></div>
    {msg_html}
    <form method="POST">
        <label>Email</label><input name="email" type="email" placeholder="you@example.com" required>
...
def reset_password(token):
    ...
    error_html = f'<div class="error">&#x26A0; {error}</div>' if error else ''
    return f"""<!DOCTYPE html><html><head>{AUTH_HEAD}<title>Reset Password - Seller AI</title>{AUTH_STYLE}</head><body>
<div class="card">
    <div class="logo"><div class="logo-icon">&#x1F511;</div><h1>New Password</h1><p>Choose a strong password</p></div>
    {error_html}
    <form method="POST">
        <label>New Password</label><input name="password" type="password" placeholder="Min 6 characters" required>
...
def login():
    ...
    msg_map = {
        'verify':   ('info',    'Account created! Please check your email to verify.'),
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, password):
            if not user.is_verified:
                error = "Please verify your email first. <a href='/resend-verification'>Resend verification &rarr;</a>"
            else:
                login_user(user)
                return redirect('/')
        else:
    ...
    return f"""<!DOCTYPE html><html><head>{AUTH_HEAD}<title>Login - Seller AI</title>{AUTH_STYLE}</head><body>
<div class="card">
    <div class="logo"><div class="logo-icon">&#x1F6D2;</div><h1>Welcome Back</h1><p>Sign in to Seller AI Dashboard</p></div>
    {msg_html}{error_html}
    <form method="POST">
        <label>Email</label><input name="email" type="email" placeholder="you@example.com" required>
    ...
    <div class="link" style="margin-top:8px;">Didn't get verification email? <a href="/resend-verification">Resend it</a></div>
</div></body></html>"""
...
def edit_product(product_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        return redirect('/')
    return (f'<h1>Edit Product</h1><form method="POST">'
...
def index():
    ...
    if request.method == 'POST':
        ...
        if file and file.filename:
            ...
            # DELETE old products and reload fresh
            Product.query.filter_by(user_id=current_user.id).delete()
            db.session.commit()

            bulk = []
            for _, row in uploaded_df.iterrows():
                title       = re.sub(r'(?i)^nan$', '', str(row.get(title_col, ''))).strip() or 'Untitled'
                description = re.sub(r'(?i)^nan$', '', str(row.get(desc_col, ''))).strip()
                category    = re.sub(r'(?i)^nan$', '', str(row.get(category_col, ''))).strip() or 'General'
                image       = fix_google_drive_link(row.get(image_col, ''))
                try:
                    raw_price = re.sub(r'[^\d.]', '', str(row.get(price_col, '0')).replace(',', ''))
                ...
                bulk.append(Product(user_id=current_user.id, title=title, description=description,
        ...
        if auto_csv:
            ...
            for _, row in auto_df.iterrows():
                ...
                try:
                    ...
                    stock = 0
                bulk2.append(Product(user_id=current_user.id, title=title, description=description,
                                     category=category, image=image, price=price, stock=stock))
    ...
    if products:
        data = [{"id": p.id, "title": p.title, "description": p.description,
                 "category": p.category, "image": p.image, "price": p.price, "stock": p.stock}
                for p in products]
        ...
        if sort_option == 'low':    df = df.sort_values(by=price_col, ascending=True)
        elif sort_option == 'high': df = df.sort_values(by=price_col, ascending=False)
        elif sort_option == 'az':   df = df.sort_values(by=title_col, ascending=True)
        elif sort_option == 'za':   df = df.sort_values(by=title_col, ascending=False)
        ...
        for i, (_, row) in enumerate(df.iterrows()):
            ...
            if category.lower() in ['google_product_category', 'category', 'type']:
                category = "General"
            ...
            if "placehold.co" in image or not image or image == "nan":
                ...
                image_warning = "<div class='image-warning'>&#x26A0; Invalid URL</div>"
                image_class   = "bad-image"
            ...
            if stock <= 5:
                stock_status = "&#x1F534; Low Stock"; stock_class = "low-stock"; low_stock_count += 1
            else:
                stock_status = "&#x1F7E2; In Stock";  stock_class = "in-stock"
            ...
            <div class="card {'duplicate-card' if is_duplicate else ''}" data-index="{i}"
                data-title="{title.replace('"', '&quot;')}" data-image="{image}"
                data-category="{category}" data-sku="{sku}"
                data-tags="{tags.replace('"', '&quot;')}" data-stock="{stock}"
                data-price="Rs. {price}" data-updated="Rs. {updated_price}"
                data-profit="Rs. {profit_per_sale}" data-inventory="Rs. {inventory_value}"
                ...
                <div class="card-body">
                    ...
                    <p><b>Stock:</b> {stock} | <b>Profit:</b> Rs. {profit_per_sale} | <b>Inv. Value:</b> Rs. {inventory_value}</p>
                    ...
                    <a href="/edit/{row['id']}" style="display:block;margin-top:5px;"><button style="width:100%;padding:11px;border-radius:10px;font-size:13px;background:#1d4ed8;border:none;color:white;cursor:pointer;">&#x270F;&#xFE0F; Edit</button></a>
                    <a href="/delete/{row['id']}" style="display:block;margin-top:5px;"><button style="width:100%;padding:11px;border-radius:10px;font-size:13px;background:#ea580c;border:none;color:white;cursor:pointer;">&#x1F5D1;&#xFE0F; Delete</button></a>
                </div>
            </div>"""
    ...
    <html><head>
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Seller AI Dashboard</title>
        ...
        <style>
            *{{box-sizing:border-box;margin:0;padding:0;}}
            ...
            .dark .stat-card,.dark .card,.dark .chart-box,.dark .modal-box{{background:#1e293b!important;color:#e2e8f0!important;}}
            .dark input,.dark select{{background:#1e293b;color:#e2e8f0;border-color:#334155;}}
            .dark .title{{color:#f1f5f9!important;}}
            .dark .price-section,.dark .modal-stat{{background:#0f172a!important;}}
            ...
            .green{{background:#059669;}}.orange{{background:#ea580c;}}.pink{{background:#db2777;}}
            ...
            .stat-card::before{{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,#1d4ed8,#7c3aed);}}
            ...
            .category-badge{{position:absolute;top:12px;left:12px;background:linear-gradient(135deg,#1d4ed8,#7c3aed);color:white;padding:5px 12px;border-radius:999px;font-size:11px;font-weight:700;z-index:5;text-transform:uppercase;}}
            ...
            .badge-blue{{background:#dbeafe;color:#1d4ed8;}}.badge-green{{background:#dcfce7;color:#059669;}}
            ...
            .modal-stat-label{{font-size:11px;color:#94a3b8;margin-top:4px;font-weight:500;text-transform:uppercase;}}
            ...
            .splash-logo{{width:90px;height:90px;background:rgba(255,255,255,0.15);border-radius:24px;display:flex;align-items:center;justify-content:center;font-size:44px;border:2px solid rgba(255,255,255,0.3);animation:logoPulse 1.2s ease-in-out infinite alternate;}}
            ...
            .splash-dots span:nth-child(2){{animation-delay:0.2s;}}.splash-dots span:nth-child(3){{animation-delay:0.4s;}}
            ...
            @media(max-width:768px){{
                ...
                .controls{{gap:8px;}}
                .controls input,.controls select,.controls button,.controls a{{width:100%!important;}}
            ...
            @media(max-width:400px){{.stats{{grid-template-columns:1fr;}}.modal-stats{{grid-template-columns:1fr;}}}}
        </style>
    </head>
    <body>
        <div id="splash">
        ...
        <div class="header">
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="controls">
                    ...
                    <a href="/clear-products" onclick="return confirm('Delete ALL products? This cannot be undone!')"><button class="orange" type="button">&#x1F5D1; Clear All Products</button></a>
        ...
        <div class="charts-container"><div class="chart-box"><canvas id="categoryChart"></canvas></div></div>
            ...
            <div class="modal-box">
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-stats">
                        ...
                        <div class="modal-stat"><div class="modal-stat-number" id="modal-profit"></div><div class="modal-stat-label">Profit/Sale</div></div>
                        <div class="modal-stat"><div class="modal-stat-number" id="modal-inventory"></div><div class="modal-stat-label">Inv. Value</div></div>
                    ...
                    <div class="modal-ai-box" id="modal-ai-box">
                        <p>Click below to generate AI insights.</p>
                        <button class="ai-btn" style="margin-top:10px;" onclick="optimizeModalProduct()">&#x2728; Generate AI Insights</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            function searchProducts(v) {{ window.location="/?search="+v; }}
            function sortProducts(v)   {{ window.location="/?sort="+v; }}
            function filterCategory(v) {{ window.location="/?category="+v; }}
            function applyBulkPrice()  {{ window.location="/?bulk="+(document.getElementById("bulkPercent").value||10); }}
            function applyDiscount()   {{ let p=document.getElementById("discountPercent").value; if(p) window.location="/?bulk=-"+p; }}
            async function optimizeProduct(index) {{
                const card=document.querySelectorAll('.card')[index];
                const btn=card.querySelector('.ai-btn');
                const old=btn.innerHTML; btn.innerHTML="&#x23F3; Optimizing..."; btn.disabled=true;
                try {{
                    const res=await fetch('/optimize',{{method:'POST',headers:{{'Content-Type':'application/json'}},
                        body:JSON.stringify({{title:card.dataset.title,description:card.dataset.desc,category:card.dataset.category}})}});
                ...
                }} catch(e){{alert("AI Error");}} finally{{btn.innerHTML=old;btn.disabled=false;}}
            }}
            function openModal(index) {{
                const card=document.querySelectorAll('.card')[index]; if(!card) return;
            ...
            async function optimizeModalProduct() {{
                ...
                }} catch(e){{box.innerHTML='<p>&#x274C; AI Error.</p>';}}
            }}
            const ctx=document.getElementById('categoryChart');
            if(ctx){{new Chart(ctx,{{type:'pie',data:{{labels:{category_labels_json},datasets:[{{label:'Products',data:{category_counts_json},borderWidth:1}}]}},options:{{layout:{{padding:20}},plugins:{{legend:{{position:'bottom'}}}}}}}});}}
        </script>
    </body></html>"""
...
def optimize_product():
    ...
    return jsonify(ai_optimize_product(data.get('title',''), data.get('description',''), data.get('category','')))
...
def export_platform(platform):
    ...
    df = pd.DataFrame([{"title":p.title,"description":p.description,"price":p.price,
                         "image":p.image,"category":p.category,"stock":p.stock} for p in products])
    if platform == "daraz":
        export_df = pd.DataFrame({"Product Name":df["title"],"Description":df["description"],
                                   "Price":df["price"],"Main Images":df["image"],"Category":df["category"]})
    elif platform == "shopify":
        export_df = pd.DataFrame({"Title":df["title"],"Body (HTML)":df["description"],
                                   "Variant Price":df["price"],"Image Src":df["image"],"Type":df["category"]})
    elif platform == "facebook":
        export_df = pd.DataFrame({"title":df["title"],"description":df["description"],"availability":"in stock",
                                   "condition":"new","price":df["price"].astype(str)+" NPR","image_link":df["image"]})
    elif platform == "instagram":
        export_df = pd.DataFrame({"id":range(1,len(df)+1),"title":df["title"],"description":df["description"],
                                   "availability":"in stock","condition":"new","price":df["price"].astype(str)+" NPR",
                                   "image_link":df["image"],"brand":"MyBrand","google_product_category":df["category"]})
    elif platform == "tiktok":
        export_df = pd.DataFrame({"Product Name":df["title"],"Product Description":df["description"],
                                   "Price":df["price"],"Main Image":df["image"],"Category":df["category"]})
    else:
        return "Invalid platform"
    return Response(export_df.to_csv(index=False), mimetype="text/csv",
